...
class Main {
  solve() {
    ...
    const arr = input.nextIntLine(n);
    _.sort(arr);
    const allSum = _.sum(arr);
    const dp = [];
    dp[-1] = {};
    dp[-1][0] = 1;
    for (let i = 0; i < n; ++i) {
      ...
      for (let key in dp[i-1]) {
        const red = key % 100000;
        const blue = Math.floor(key / 100000);
        const count = dp[i-1][key];
        const cur = arr[i];
        dp[i][(blue+cur) * 100000+red] = dp[i][(blue+cur) * 100000+red] == null ? 0 : dp[i][(blue+cur) * 100000+red];
        dp[i][(blue+cur) * 100000+red] += count;
        dp[i][(blue+cur) * 100000+red] %= MOD.mod;
        dp[i][(blue) * 100000+(red+cur)] = dp[i][(blue) * 100000+(red+cur)] == null ? 0 : dp[i][(blue) * 100000+(red+cur)];
        dp[i][(blue) * 100000+(red+cur)] += count;
        dp[i][(blue) * 100000+(red+cur)] %= MOD.mod;
        dp[i][key] = dp[i][key] == null ? 0 : dp[i][key];
        dp[i][key] += count;
        dp[i][key] %= MOD.mod;
      }
    }
    let lasts = dp[n-1];
    let sum = 0;
    for (let key in lasts) {
      const red = key % 1e5;
      const blue = Math.floor(key / 1e5);
      const green = allSum - red - blue;
      if (Math.max(red,blue,green) * 2 < _.sum([red,blue,green])) {
        sum += lasts[key];
        sum %= MOD.mod;
      }
    }
